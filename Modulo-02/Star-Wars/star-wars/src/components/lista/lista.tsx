...
export const Lista = <T extends GenericSchema>(props: ListaProps) => {
  ...
  const { data, isLoading, pageData, error } = useLista<T>(
    page,
    expressaoBusca,
    controller
  );
  ...
  const handleRowClick = useMemo(
    () => (param: GridRowParams) => {
      history.push(`/detail/${controller}/${param.row.id}`);
    },
    [history, controller]
  );
  ...
  if (!columnData) {
    history.push(
      '/erro/Não estamos preparados para mostrar esse tipo de informação'
    );
    return <TelaErro />;
  }
  const columns: GridColDef[] = mediaQuery ? columnData : [columnData[0]];
        ...
        <Grid item>
          <div style={{ height: 400, width: '100%' }}>
            <DataGrid
              autoHeight
              rows={data}
              rowCount={pageData?.totalRegistros}
              loading={isLoading}
              columns={columns}
              page={page}
              pageSize={10}
