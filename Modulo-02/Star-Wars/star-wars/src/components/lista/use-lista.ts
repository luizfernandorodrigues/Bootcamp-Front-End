 ...
 */
export const useLista = <T extends GenericSchema>(
  page: number,
  ...
  const controller = useMemo(
    () =>
      genericController<T>(controllerName).getByPartialName(
        expressaoBusca,
        page
      ),
    [controllerName, expressaoBusca, page]
  );
  const { result, isLoading, error } = useAsync<PagebleResponse<T>>(
    controller,
    [page, expressaoBusca, controllerName]
  );

  useEffect(() => {
    if (!result) return;
    const newData = result.dados.map((dado) => {
